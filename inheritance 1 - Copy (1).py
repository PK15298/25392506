...
label_select_add_ons=Label(root,text="Select Add-Ons",font=("times",15))
label_select_add_ons.place(relx=0.08, rely=0.5,anchor=CENTER)
...
class child2(parent):
    ...
    def get_final_amount(self):
        new_dish=dish_dropdown.get()
        add_ons=toppings_dropdown.get()
        parent.final_amount(new_dish,add_ons)
        
child1_object =child1(dish_dropdown.get())
child1_object.get_order()
child2_object = child2(toppings_dropdown.get(),dish_dropdown.get())
child2_object.get_final_amount()  

btn_add_ons = Button(root,text="Check Add-Ons",command=child1_object.get_order,bg="Blue", fg="white",relief = FLAT)
btn_add_ons.place(relx=0.06,rely=0.3,anchor=CENTER)
